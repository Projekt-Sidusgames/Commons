 ...
 * <p>
 * <p>
 * Random random = ... Map<String, Double> stringWeights = new Hash<>();<br> stringWeights.put("a", 4d);<br> stringWeights.put("b", 3d);<br> stringWeights.put("c", 12d);<br> stringWeights.put("d", 1d);<br>
 * <p>
 * RandomSelector<String> selector = RandomSelector.weighted(stringWeights.keySet(), s -> stringWeights.get(s));<br> List<String> selection = new ArrayList<>();<br> for (int i = 0; i < 10; i++) {<br> selection.add(selector.next(random));<br> }
 *
 * @param <T>
 */
public final class RandomSelector<T> {
   ...
   *
   * <p>
   * A copy of <tt>elements</tt> is kept, so any modification to
   * <tt>elements</tt> will not be reflected in returned values.
   *
   * @param <T>
   * @param elements
   * @return -
  ...
  public static <T> RandomSelector<T> uniform(final Collection<T> elements) throws IllegalArgumentException {
    Preconditions.checkNotNull(elements, "collection must not be null");
  ...
  /**
   * Creates a random selector among <tt>elements</tt> where the elements have a weight defined by <tt>weighter</tt>.
   *
   * <p>
   * A copy of <tt>elements</tt> is kept, so any modification to
   * <tt>elements</tt> will not be reflected in returned values.
   ...
   * @param weighter
   * @return -
   * @throws IllegalArgumentException if <tt>elements</tt> is empty or if <tt>weighter</tt> returns a negative value or <tt>0</tt>.
   */
  public static <T> RandomSelector<T> weighted(final Collection<T> elements, final ToDoubleFunction<? super T> weighter) throws IllegalArgumentException {
    Preconditions.checkNotNull(elements, "elements must not be null");
    Preconditions.checkNotNull(weighter, "weighter must not be null");
  ...
  /**
   * Returns a stream of elements using <tt>random</tt>. The stream must use a terminal operation to become closed and free the resources it's been using.
   *
   * <p>
  ...
  public Stream<T> stream(final Random random) {
    ...
    return StreamSupport.stream(Spliterators.spliteratorUnknownSize(new BaseIterator(random), IMMUTABLE | ORDERED), false);
  }
  ...
  private class BaseIterator implements Iterator<T> {
    ...
    @Override
    public T next() {
      return RandomSelector.this.next(random);
    }
  }
